...
const OPENAI_BASE = import.meta.env.VITE_OPENAI_API_BASE_URL ?? "https://api.openai.com/v1";
...
const OPENAI_MODELS = {
  questionScan: "deepseek-ai/DeepSeek-V4-Flash",
  rubricGeneration: "deepseek-ai/DeepSeek-V3.2",
  grading: "MiniMaxAI/MiniMax-M2.5",
};
import {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      ...
      <div style={{ background: T.card, borderRadius: T.r, border: `1px solid ${T.border}`, padding: "20px 22px" }}>
        ...
        <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: "100%", height: "auto" }}>
          ...
          {distTicks.map((t, i) => {
            const y = padT + plotH - (t / maxCount) * plotH; return (<g key={i}>
          ...
          {buckets.map((c, i) => {
            const bh = (c / maxCount) * plotH; const bx = padL + i * (barW + barGap); const by = padT + plotH - bh; return (<g key={i}>
      ...
      <div style={{ background: T.card, borderRadius: T.r, border: `1px solid ${T.border}`, padding: "20px 22px" }}>
        ...
        <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: "100%", height: "auto" }}>
          ...
          {errTicks.map((t, i) => {
            const y = padT + plotH - (t / errMax) * plotH; return (<g key={i}>
              <line x1={padL} y1={y} x2={chartW - padR} y2={y} stroke={T.border} strokeDasharray="3,3" />
              <text x={padL - 6} y={y + 3} fontSize="10" fill={T.textSec} textAnchor="end">{t}</text>
            </g>);
          })}
          {perQ.map((p, i) => {
            const bh = (p.errorRate / errMax) * plotH; const bx = padL + i * (errBarW + barGap); const by = padT + plotH - bh; const isHighest = hardest && p.q.id === hardest.q.id; return (<g key={i}>
...
function buildSubmissions(qs, diffMap, opts, dateBase, statusSeed = 0) {
  ...
  return MOCK_NAMES.map((name, i) => {
    ...
    const grades = graded ? qs.map(q => {
      ...
      return {
        qId: q.id, qNum: q.num, total, maxPts: q.pts,
        criteria: [
    ...
    return {
      name, email, submittedAt, status,
      grades,
      totalScore, maxScore,
      skill
    }; // keep skill for debugging / future use
  });
}
      ...
      {(assignment.questions || []).map(q => {
        ...
        return (<div key={q.id} style={{ background: T.card, borderRadius: T.r, border: `1px solid ${T.border}`, padding: "24px 28px" }}>
          ...
          {criteria.length > 0 && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 10, marginBottom: 16 }}>
            {criteria.map(c => {
              const tone = criterionTone(c); return (<div key={c.title} style={{ background: tone.bg, border: `1px solid ${tone.color}24`, borderRadius: T.rs, padding: "11px 13px" }}>
...
- If a value is unknown: pts=1, type="free", answerKey="See key".`;
      ...
      const res = await fetch(`${OPENAI_BASE}/chat/completions`, {
        ...
        body: JSON.stringify({
          model: OPENAI_MODELS.questionScan,
          max_tokens: 16384,
  ...
  const generateRubrics = async () => {
    ...
    for (const q of questions) {
      ...
      try {
        ...
        const r = await fetch(`${OPENAI_BASE}/chat/completions`, { method: "POST", headers: OPENAI_HEADERS, body: JSON.stringify({ model: OPENAI_MODELS.rubricGeneration, max_tokens: 600, messages: [{ role: "user", content: prompt }] }) });
...
Confidence calibration:
 ...
 - No student text found at all → <0.4 and say so in confidenceReason.`;
          const r = await fetch(`${OPENAI_BASE}/chat/completions`, { method: "POST", headers: OPENAI_HEADERS, body: JSON.stringify({ model: OPENAI_MODELS.grading, max_tokens: 1200, messages: [{ role: "user", content: prompt }] }) });
  ...
  const S2 = () => (<div>
    ...
    {rubricGen && <div style={{ background: T.card, borderRadius: T.r, border: `1px solid ${T.border}`, padding: 24, boxShadow: T.sh }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{questions.map(q => {
        const p = genProgress.find(x => x.qId === q.id); return (<div key={q.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: T.rs, background: p?.status === "done" ? T.greenLight : p?.status === "gen" ? T.primaryLight : "#F8F9FC", border: `1px solid ${p?.status === "done" ? T.green + "30" : T.border}` }}>
      ...
      return (<div>
        ...
        {(() => {
          ...
          return (<div style={{ borderRadius: T.r, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            ...
            {visibleGrades.map(({ g, gi }) => {
              const isOpen = expandedQ === gi; const sp = g.maxPts > 0 ? g.total / g.maxPts : 0; const sc = sp >= 0.8 ? T.green : sp >= 0.5 ? T.orange : T.red;
              const key = `${expandedStudent}-${g.qId}`;
              ...
              return (<div key={gi} style={{ borderBottom: gi < r.grades.length - 1 ? `1px solid ${T.border}` : "none", background: tier === "low" && !appr ? "#FEF7F7" : "transparent" }}>
                ...
                {isOpen && <div style={{ padding: "0 20px 16px 52px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {g.criteria.map((c, ci) => {
                      const full = c.earned === c.maxPts; const zero = c.earned === 0; return (<div key={ci} style={{ borderRadius: T.rs, border: `1px solid ${full ? T.green + "30" : zero ? T.red + "20" : T.orange + "30"}`, background: full ? T.greenLight : zero ? "#FEF2F2" : T.orangeLight, padding: "10px 14px" }}>
  ...
  const S5 = () => {
    if (!gradingResults) return null; const examTotal = questions.reduce((s, q) => s + q.pts, 0);
    return (<div>
